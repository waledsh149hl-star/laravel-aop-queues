...
// ملف الهجوم (attack4.php) - لإثبات نجاح القفل الموزع المشترك
$productId = 4; 
$totalRequests = 20; 
$mh = curl_multi_init();
$curl_handles = [];
...
for ($i = 1; $i <= $totalRequests; $i++) {
    ...
    // الرابط المحمي بـ lockForUpdate أو الآلية الآمنة:
    $url = "http://127.0.0.1:{$port}/api/buy1/{$productId}";
...
foreach ($curl_handles as $index => $item) {
    ...
    // جلب نص الرد الراجع من لارافل
    $response = curl_multi_getcontent($ch);
    
    // تحديد نجاح أو فشل العملية برمجياً
